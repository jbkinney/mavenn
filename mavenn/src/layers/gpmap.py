...
class Multi_PairwiseGPMapLayer(GPMapLayer):
    """Represents a pairwise G-P map."""
    ...
    def __init__(self, number_latent_nodes, *args, **kwargs):
        ...
        # Create mask
        ls = np.arange(self.L).astype(int)
        ls1 = np.tile(ls.reshape([1, 1, self.L, 1, 1, 1]),
                      [1, 1, 1, self.C, self.L, self.C])
        ls2 = np.tile(ls.reshape([1, 1, 1, 1, self.L, 1]),
                      [1, 1, self.L, self.C, 1, self.C])
        if self.mask_type == 'pairwise':
            self.mask = (ls2 - ls1 >= 1)
        elif self.mask_type == 'neighbor':
            self.mask = (ls2 - ls1 == 1)
    ...
    def build(self, input_shape):
        ...
        # Define theta_lclc parameters
        theta_lclc_shape = (1, self.number_latent_nodes, self.L, self.C, self.L, self.C)
        theta_lclc_init = np.random.randn(*theta_lclc_shape)/np.sqrt(self.L**2)
        theta_lclc_init *= self.mask
        self.theta_lclc = self.add_weight(name='theta_lclc',
                                          shape=theta_lclc_shape,
    ...
    def call(self, x_lc):
        ...
        phi = self.theta_0
        phi = phi + tf.reshape(K.sum(self.theta_lc *
                                     tf.reshape(x_lc, [-1, 1, self.L, self.C]),
                                     axis=[2, 3]),
                               shape=[-1, self.number_latent_nodes])
        ...
        phi = phi + tf.reshape(K.sum(self.theta_lclc *
                                     self.mask *
                                     tf.reshape(x_lc,
                                         [-1, 1, self.L, self.C, 1, 1]) *
                                     tf.reshape(x_lc,
                                         [-1, 1, 1, 1, self.L, self.C]),
                                     axis=[2, 3, 4, 5]),
                               shape=[-1, self.number_latent_nodes])
