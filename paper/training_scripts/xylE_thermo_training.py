...
def main(args):
    ...
    # modify the params_dict gpmap_kwargs
    re = args.regularizer
    params_dict["gpmap_kwargs"]["regularizer"] = re
    params_dict["gpmap_kwargs"]["theta_regularization"] = re

    # Just print to check elzar specified the var correctly
    print(f'lr: {lr}, bs:{bs}, re:{re}')
    ...
    # Train model
    model.fit(learning_rate=lr, epochs=ep,
              batch_size=bs,
              **params_dict["fit_params"])

    # Save model to file
    model_name = f"../models/xylE_{model_type}_lr_{lr}_bs_{bs}_re_{re}_{date_str}"
    print(f'model is saved in {model_name}')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ep", "--epochs", default=500, type=int, help="Number of epochs"
    )
    parser.add_argument(
        "-lr", "--learning_rate", default=5e-4, type=float, help="Number of epochs"
    )

    parser.add_argument(
        "-bs", "--batch_size", default=1000, type=int, help="Batch size"
    )

    parser.add_argument(
        "-re", "--regularizer", default=1e-1, type=float, help="Regularization in gpmap"
    )
