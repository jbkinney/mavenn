...
def MaximizeMI_memsaver(
        raveledmat,df,emat_0,db=None,burnin=1000,iteration=30000,thin=10,
        runnum=0,verbose=False):
    ...
    def emat(p=pymcdf,value=emat_0):         
        p['val'] = raveledmat.multiply(emat_0).sum(axis=1)                   
        df_sorted = p.sort(columns='val')
        df_sorted.reset_index(inplace=True)
        n_seqs = raveledmat.shape[0]     
        MI = EstimateMutualInfoforMImax.alt4(df_sorted.copy())  # New and improved
        return n_seqs*MI
    if db:
        dbname = db + '_' + str(runnum) + '.sql'
    ...
    M.sample(iteration,thin=thin)
    emat_list = M.trace('emat')[burnin:]
    emat_fixed = np.zeros(len(seq_dict),len(emat_list.shape[1])/(len(seq_dict)-1),emat_list.shape[0])
    if modeltype == 'MAT':
        for i in range(emat_list.shape[0]):
            emat_temp = utils.emat_typical_parameterization(emat_list[i,:],len(seq_dict),wtseq,seq_dict)
            emat_new[:,:,i] = gauge_fix.fix_matrix(np.transpose(emat_temp))
    elif modeltype == 'NBR':
        for i in range(emat_list.shape[0]):
            emat_temp = utils.emat_typical_parameterization(emat_list[i,:],len(seq_dict),wtseq,seq_dict)
            emat_new[:,:,i] =  gauge_fix.fix_neighbor(np.transpose(emat_temp))
    return np.mean(emat_new,axis=2)
...
def main(df,lm='memsaver',modeltype='MAT',LS_means_std=None,\
    ...
    if lm == 'LS':
        ...
        raveledmat,batch,sw = utils.genweightandmat(
                                  df,wt_seq_dict_list,dicttype,means=means,modeltype=modeltype)
    ...
    if lm == 'memsaver':
        if modeltype == 'NBR':
            ...
            raveledmat,batch,sw = utils.genweightandmat(
                                  df.copy(),wt_seq_dict_list,dicttype,means='None',modeltype=modeltype)
        print wtseq
        print df.seq[1]
        print raveledmat[1,:].todense()
        print wt_seq_dict_list[0:4]
        ...
        if initialize == 'Rand':
            ...
            emat_0 = Compute_Least_Squares(raveledmat,batch,sw,alpha=.5)
        df['ct'] = df[col_headers].sum(axis=1)
        
            #pymc doesn't take sparse mat        
        emat = MaximizeMI_memsaver(
                raveledmat,df,emat_0,db=db,iteration=iteration,burnin=burnin,
                thin=thin,runnum=runnum,verbose=verbose)
    ...
    elif (lm=='LS'): #must be Least squares
        emat_typical = utils.emat_typical_parameterization(emat,len(seq_dict),wtseq,seq_dict)        
        if modeltype == 'NBR':
