...
def fix_neighbor(neighbor_model, verbose=False, rcond=1.0E-10):
    ...
    # Now Fix it such that sum of variances = 1
    sum_of_var = np.sum(np.var(fixed_neighbor_model,axis=1))
    fixed_neighbor_model = fixed_neighbor_model/np.sqrt(sum_of_var)
